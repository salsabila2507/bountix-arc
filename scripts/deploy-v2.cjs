...
function requireAddress(name) {
  ...
  if (!value || !hre.ethers.isAddress(value) || value === hre.ethers.ZeroAddress) {
    throw new Error(`Set ${name} to a non-zero address before deploying V2.`);
  }
  return value;
}
...
async function main() {
  ...
  console.log("Deploying BountixEscrowV2 to Base mainnet...\n");
  ...
  const resolverAddress = optionalAddress("V2_RESOLVER_ADDRESS", deployer.address);
  const treasuryAddress = requireAddress("V2_TREASURY_ADDRESS");
  ...
  const BountixEscrowV2 = await hre.ethers.getContractFactory("BountixEscrowV2");

  console.log("Estimating deployment gas...");
  const deployTx = await BountixEscrowV2.getDeployTransaction(
    BASE_MAINNET_USDC,
    resolverAddress,
    treasuryAddress,
  );
  ...
  const escrow = await BountixEscrowV2.deploy(
    BASE_MAINNET_USDC,
    resolverAddress,
    treasuryAddress,
  );
